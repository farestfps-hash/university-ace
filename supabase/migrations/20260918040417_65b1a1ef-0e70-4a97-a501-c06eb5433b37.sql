ALTER TABLE public.profiles ALTER COLUMN portfolio_public SET DEFAULT true;
UPDATE public.profiles SET portfolio_public = true WHERE portfolio_public = false;
CREATE OR REPLACE FUNCTION public.get_leaderboard(_country text DEFAULT NULL::text)
 RETURNS TABLE(user_id uuid, is_public boolean, full_name text, high_school text, grade_level text, target_major text, target_countries text[], gpa_unweighted numeric, sat_score integer, act_score integer, unt_score integer, nuet_score integer, holistic_score integer, summary text, countries jsonb, strengths jsonb, evaluated_at timestamp with time zone)
 LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
  SELECT
    p.id, p.portfolio_public,
